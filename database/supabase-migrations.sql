...
-- Tabela: CONTATOS (CRM)
-- Nota: Se já existe, fazer alterações incrementais
...
CREATE POLICY "Users can view their cash flow"
...
CREATE POLICY "Users can view their progress"
  ON progresso_semanal FOR SELECT
  USING (auth.uid() = user_id);

CREATE POLICY "Users can view their satisfaction data"
  ON satisfacao FOR SELECT
  USING (auth.uid() = user_id);
